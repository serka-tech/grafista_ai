...
approvalsRouter.post('/content-ideas/:id/approve', async (req: Request, res: Response) => {
...
approvalsRouter.post('/content-ideas/:id/reject', async (req: Request, res: Response) => {
  const idea = await store.contentIdeas.getById(req.params.id);
  if (!idea) return res.status(404).json({ error: 'Content idea not found' });
